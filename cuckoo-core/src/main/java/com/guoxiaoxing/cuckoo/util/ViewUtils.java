...
//                    viewText =  ViewUtils.getViewId(imageView);
                    }
    ...
    public static void getFragmentNameFromView(View view, JSONObject properties) {
        try {
            if (view != null) {
                String fragmentName = (String) view.getTag(R.id.cuckoo_tag_view_fragment_name);
                String fragmentName2 = (String) view.getTag(R.id.cuckoo_tag_view_fragment_name2);
                if (!TextUtils.isEmpty(fragmentName2)) {
                    fragmentName = fragmentName2;
                }
                if (!TextUtils.isEmpty(fragmentName)) {
                    String screenName = properties.getString(AspectjConstants.SCREEN_NAME);
                    if (!TextUtils.isEmpty(fragmentName)) {
                        properties.put(AspectjConstants.SCREEN_NAME, String.format(Locale.CHINA, "%s|%s", screenName, fragmentName));
                    } else {
        ...
        try {
            if (context != null) {
                if (context instanceof Activity) {
                    ...
                    if (view != null) {
                        Object object = view.getTag(R.id.cuckoo_tag_view_activity);
                        if (object != null) {
    ...
    public static String getViewId(View view) {
        ...
        try {
            idString = (String) view.getTag(R.id.cuckoo_tag_view_id);
    ...
    public static boolean isViewIgnored(View view) {
        try {
            ...
            //View 被忽略
            if ("1".equals(view.getTag(R.id.cuckoo_tag_view_ignored))) {
                return true;
            }
            ...
            if (activity != null) {
                try {
                    ...
                    if (Build.VERSION.SDK_INT >= 11) {
                        String toolbarTitle = DataUtils.getToolbarTitle(activity);
