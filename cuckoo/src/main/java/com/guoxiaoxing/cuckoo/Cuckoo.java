...
import com.guoxiaoxing.cuckoo.annotation.ignore.IgnoreTrackAppClick;
import com.guoxiaoxing.cuckoo.annotation.ignore.IgnoreTrackAppViewScreen;
import com.guoxiaoxing.cuckoo.annotation.ignore.IgnoreTrackAppViewScreenAndAppClick;
import com.guoxiaoxing.cuckoo.db.SharedPreferencesLoader;
import com.guoxiaoxing.cuckoo.debug.DebugTracking;
import com.guoxiaoxing.cuckoo.event.EventTimer;
import com.guoxiaoxing.cuckoo.event.EventType;
import com.guoxiaoxing.cuckoo.exception.ExceptionHandler;
import com.guoxiaoxing.cuckoo.exception.InvalidDataException;
import com.guoxiaoxing.cuckoo.model.AppWebViewInterface;
import com.guoxiaoxing.cuckoo.model.PersistentDistinctId;
import com.guoxiaoxing.cuckoo.model.PersistentFirstDay;
import com.guoxiaoxing.cuckoo.model.PersistentFirstStart;
import com.guoxiaoxing.cuckoo.model.PersistentFirstTrackInstallation;
import com.guoxiaoxing.cuckoo.model.PersistentFirstTrackInstallationWithCallback;
import com.guoxiaoxing.cuckoo.model.PersistentLoginId;
import com.guoxiaoxing.cuckoo.model.PersistentSuperProperties;
import com.guoxiaoxing.cuckoo.model.ScreenAutoTracker;
import com.guoxiaoxing.cuckoo.util.ConfigurationChecker;
import com.guoxiaoxing.cuckoo.util.CuckooActivityLifecycleCallbacks;
...
import com.guoxiaoxing.cuckoo.vtrack.VTrackUnsupported;
...
public class Cuckoo {
    ...
    Cuckoo(Context context, String serverURL, String configureURL,
        ...
        try {
            DataUtils.cleanUserAgent(mContext);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            if (debugMode == DebugMode.DEBUG_OFF) {
                ENABLE_LOG = configBundle.getBoolean("com.guoxiaoxing.cuckoo.android.EnableLogging",
                        false);
            } else {
                ENABLE_LOG = configBundle.getBoolean("com.guoxiaoxing.cuckoo.android.EnableLogging",
                        true);
            }
            SHOW_DEBUG_INFO_VIEW = configBundle.getBoolean("com.guoxiaoxing.cuckoo.android.ShowDebugInfoView",
                    true);

            mFlushInterval = configBundle.getInt("com.guoxiaoxing.cuckoo.android.FlushInterval",
                    15000);
            mFlushBulkSize = configBundle.getInt("com.guoxiaoxing.cuckoo.android.FlushBulkSize",
                    100);
            mAutoTrack = configBundle.getBoolean("com.guoxiaoxing.cuckoo.android.AutoTrack",
                    false);
            mEnableVTrack = configBundle.getBoolean("com.guoxiaoxing.cuckoo.android.VTrack",
                    true);
            if (TextUtils.isEmpty(configureURL)) {
                mEnableVTrack = false;
            }
            mEnableAndroidId = configBundle.getBoolean("com.guoxiaoxing.cuckoo.android.AndroidId",
                    false);
            mEnableButterknifeOnClick = configBundle.getBoolean("com.guoxiaoxing.cuckoo.android.ButterknifeOnClick",
                    false);
            mMainProcessName = configBundle.getString("com.guoxiaoxing.cuckoo.android.MainProcessName");

            //打开debug模式，弹出提示
            if (mDebugMode != DebugMode.DEBUG_OFF && DataUtils.isMainProcess(mContext.getApplicationContext(), mMainProcessName)) {
            ...
            if (Build.VERSION.SDK_INT >= VTRACK_SUPPORTED_MIN_API
                ...
                String resourcePackageName =
                        configBundle.getString("com.guoxiaoxing.cuckoo.android.ResourcePackageName");
                if (null == resourcePackageName) {
                    resourcePackageName = context.getPackageName();
                }

                mVTrack = new VTrackSupported(mContext, resourcePackageName);
            } else {
                if (debugMode != DebugMode.DEBUG_OFF) {
            ...
            throw new RuntimeException("Can't configure Cuckoo with package name " + packageName,
                    e);
        }
        ...
        final String prefsName = "com.guoxiaoxing.cuckoo.Cuckoo";
        ...
        if (mEnableAndroidId) {
            try {
                String androidId = DataUtils.getAndroidID(mContext);
                if (DataUtils.isValidAndroidId(androidId)) {
                    identify(androidId);
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            ...
            app.registerActivityLifecycleCallbacks(new CuckooActivityLifecycleCallbacks(this, mFirstStart, mMainProcessName));
        }
...

        {
            ...
            if (DataUtils.checkHasPermission(context, "android.permission.READ_PHONE_STATE")) {
                try {
                    ...
                    if (telephonyManager != null) {
                        ...
                        if (!TextUtils.isEmpty(operatorString)) {
                            deviceInfo.put("$carrier", DataUtils.operatorToCarrier(operatorString));
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
     ...
     */
    public static Cuckoo with(Context context) {
        if (null == context) {
            return null;
        }
        synchronized (sInstanceMap) {
            final Context appContext = context.getApplicationContext();
            Cuckoo instance = sInstanceMap.get(appContext);

            if (null == instance) {
                Log.i(TAG, "The static method with(context, serverURL, configureURL, "
                        + "vtrackServerURL, debugMode) should be called before calling with()");
            }
            return instance;
        }
    }
     ...
     *                  {@link Cuckoo.DebugMode}
     * @return SensorsDataAPI单例
     */
    public static Cuckoo with(Context context, String serverURL, DebugMode debugMode) {
        return with(context, serverURL, null, null, debugMode);
    }
     ...
     * @return SensorsDataAPI单例
     */
    public static Cuckoo with(Context context, String serverURL, String
        ...
        synchronized (sInstanceMap) {
            ...
            if (null == instance && ConfigurationChecker.checkBasicConfiguration(appContext)) {
                instance = new Cuckoo(appContext, serverURL, configureUrl, null, debugMode);
     ...
     * @return SensorsDataAPI单例
     */
    public static Cuckoo with(Context context, String serverURL,
        ...
        synchronized (sInstanceMap) {
            ...
            Cuckoo instance = sInstanceMap.get(appContext);
            if (null == instance && ConfigurationChecker.checkBasicConfiguration(appContext)) {
                instance = new Cuckoo(appContext, serverURL, configureURL, vtrackServerURL,
                        debugMode);
                sInstanceMap.put(appContext, instance);
            }
    ...
    public static Cuckoo with() {
        synchronized (sInstanceMap) {
            if (sInstanceMap.size() > 0) {
                Iterator<Cuckoo> iterator = sInstanceMap.values().iterator();
                if (iterator.hasNext()) {
     ...
     * 打开 SDK 自动追踪
     * <p>
     * 该功能自动追踪 App 的一些行为，指定哪些 AutoTrack 事件被追踪.
     * <p>
     * 该功能仅在 API 14 及以上版本中生效，默认关闭
     *
    ...
    public interface InstanceProcessor {
        public void process(Cuckoo m);
    }

    /* package */
    public static void allInstances(InstanceProcessor processor) {
        synchronized (sInstanceMap) {
            for (final Cuckoo instance : sInstanceMap.values()) {
    ...
    public boolean isActivityAutoTrackAppViewScreenIgnored(Class<?> activity) {
        ...
        if (activity.getAnnotation(IgnoreTrackAppViewScreenAndAppClick.class) != null) {
            return true;
        }

        if (activity.getAnnotation(IgnoreTrackAppViewScreen.class) != null) {
            return true;
        }
    ...
    public boolean isActivityAutoTrackAppClickIgnored(Class<?> activity) {
        ...
        if (activity.getAnnotation(IgnoreTrackAppClick.class) != null) {
        return true;
    }
    ...
    public void setViewID(View view, String viewID) {
        if (view != null && !TextUtils.isEmpty(viewID)) {
            view.setTag(R.id.cuckoo_tag_view_id, viewID);
        }
    }
        ...
        try {
            if (view != null && !TextUtils.isEmpty(viewID)) {
                if (view.getWindow() != null) {
                    view.getWindow().getDecorView().setTag(R.id.cuckoo_tag_view_id, viewID);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void setViewActivity(View view, Activity activity) {
        try {
            ...
            view.setTag(R.id.cuckoo_tag_view_activity, activity);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void setViewFragmentName(View view, String fragmentName) {
        try {
            ...
            view.setTag(R.id.cuckoo_tag_view_fragment_name2, fragmentName);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void ignoreView(View view) {
        if (view != null) {
            view.setTag(R.id.cuckoo_tag_view_ignored, "1");
        }
    }
    ...
    public void setViewProperties(View view, JSONObject properties) {
        ...
        view.setTag(R.id.cuckoo_tag_view_properties, properties);
    }
        ...
        try {
            ...
            if (firstTrackInstallation) {
                try {
                    ...
                    if (!DataUtils.hasUtmProperties(properties)) {
                        ...
                        for (Map.Entry<String, String> entry : utmMap.entrySet()) {
                            if (entry != null) {
                                String utmValue = DataUtils.getApplicationMetaData(mContext, entry.getKey());
                    ...
                    if (!DataUtils.hasUtmProperties(properties)) {
                        String installSource = String.format("android_id=%s##imei=%s##mac=%s",
                                DataUtils.getAndroidID(mContext),
    ...
    public void trackViewScreen(String url, JSONObject properties) {
        try {
            if (!TextUtils.isEmpty(url) || properties != null) {
                ...
                mLastScreenUrl = url;
                if (properties != null) {
                    DataUtils.mergeJSONObject(properties, trackProperties);
                }
    ...
    public void trackViewScreen(android.app.Fragment fragment) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= 11) {
                ...
                if (activity != null) {
                    String activityTitle = DataUtils.getActivityTitle(activity);
    ...
    public void registerSuperProperties(JSONObject superProperties) {
        try {
            ...
            synchronized (mSuperProperties) {
                ...
                DataUtils.mergeJSONObject(superProperties, properties);
    ...
    public void trackEventFromH5(String eventInfo) {
        try {
            ...
            if (eventType.isTrack()) {
                ...
                synchronized (mSuperProperties) {
                    JSONObject superProperties = mSuperProperties.get();
                    DataUtils.mergeJSONObject(superProperties, propertiesObject);
                }
        ...
        try {
            ...
            try {
                ...
                if (eventType.isTrack()) {
                    ...
                    synchronized (mSuperProperties) {
                        JSONObject superProperties = mSuperProperties.get();
                        DataUtils.mergeJSONObject(superProperties, sendProperties);
                    }
