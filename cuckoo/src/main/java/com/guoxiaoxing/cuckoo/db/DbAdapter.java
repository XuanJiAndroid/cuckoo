package com.guoxiaoxing.cuckoo.db;
...
public class DbAdapter {
    ...
    private long getMaxCacheSize(Context context) {
        try {
            return Cuckoo.with(context).getMaxCacheSize();
        } catch (Exception e) {
            e.printStackTrace();
            return 32 * 1024 * 1024;
        }
    ...
    public int addJSON(JSONObject j, Table table) {
        ...
        try {
            if (!belowMemThreshold()) {
                LogUtils.i(TAG, "There is not enough space left on the device to store events, so will delete some old events");
    ...
    public String[] generateDataString(Table table, int limit) {
        ...
        try {
            ...
            LogUtils.i(TAG, "Could not pull records for SensorsData out of database " + tableName
                    + ". Waiting to send.", e);
            last_id = null;
            data = null;
        } finally {
