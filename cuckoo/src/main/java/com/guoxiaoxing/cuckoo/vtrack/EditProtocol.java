...
import com.guoxiaoxing.cuckoo.model.Caller;
import com.guoxiaoxing.cuckoo.model.PropertyDescription;
...
public class EditProtocol {
    ...
    private static final List<ViewPathFinder.PathElement> NEVER_MATCH_PATH =
            Collections.<ViewPathFinder.PathElement>emptyList();
    ...
    public ViewVisitor readEventBinding(JSONObject source, ViewVisitor.OnEventListener listener)
        ...
        try {
            ...
            final List<ViewPathFinder.PathElement> path = readPath(pathDesc, mResourceIds);
    ...
    private List<ViewPathFinder.PathElement> readPath(JSONArray pathDesc, ResourceIds idNameToId)
            throws JSONException {
        final List<ViewPathFinder.PathElement> path = new ArrayList<ViewPathFinder.PathElement>();
        ...
        for (int i = 0; i < pathDesc.length(); i++) {
            ...
            if ("shortest".equals(prefixCode)) {
                prefix = ViewPathFinder.PathElement.SHORTEST_PREFIX;
            } else if (null == prefixCode) {
                prefix = ViewPathFinder.PathElement.ZERO_LENGTH_PREFIX;
            } else {
                LogUtils.i(TAG, "Unrecognized prefix type \"" + prefixCode + "\". No views will be matched");
            ...
            path.add(new ViewPathFinder.PathElement(prefix, targetViewClass, targetIndex, targetId));
        }
