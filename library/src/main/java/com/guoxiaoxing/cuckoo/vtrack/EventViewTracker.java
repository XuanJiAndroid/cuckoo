...
import com.guoxiaoxing.cuckoo.vtrack.ViewVisitor;
...
class EventViewTracker implements ViewVisitor.OnEventListener {

    public EventViewTracker(Context context, Handler homeHandler) {
        mContext = context;
    ...
    public void OnEvent(View v, EventInfo eventInfo, boolean debounce) {
        ...
        try {
            ...
            LogUtils.i(TAG, "Can't format properties from view due to JSON issue", e);
        }
        ...
        if (debounce) {
            ...
            Cuckoo.with(mContext).track(eventInfo.mEventName, properties);
        }
    }
    ...
    private static String TAG = "SA.EventViewTracker";
}
